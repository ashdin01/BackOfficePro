...
from views.purchase_orders.po_email_worker import _PoEmailWorker
...
class PODetail(BaseView):
    ...
    def _email_po(self):
        ...
        # Safety: if the worker thread finishes without emitting success/failure
        # (e.g. a BaseException that bypasses the try/except), close the dialog
        # so the event loop is not blocked indefinitely.
        self._email_worker.finished.connect(
            lambda: progress.accept() if not progress.isHidden() else None
        )
